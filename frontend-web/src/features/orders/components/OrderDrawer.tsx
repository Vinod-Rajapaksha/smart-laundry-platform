...
import { X, Clock, Package, MapPin, CreditCard, ChevronRight } from "lucide-react";
...
export default function OrderDrawer({ order, onClose, onUpdateStatus, loading }: OrderDrawerProps) {
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-lg bg-white shadow-2xl z-50 overflow-y-auto font-poppins animate-in slide-in-from-right duration-300">
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 p-6 space-y-8">
            ...
            <section className="bg-slate-50 p-6 rounded-2xl space-y-4">
              <div className="flex items-center gap-3 text-slate-600">
                <Clock size={16} />
                <p className="text-sm font-medium">Placed on {format(new Date(order.createdAt), "PPpp")}</p>
              </div>
                ...
                <div className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 font-mono tracking-tighter uppercase">{order.serviceId}</span>
                    </div>
                  </div>
          ...
          <div className="p-6 border-t border-slate-100 flex gap-4 bg-slate-50/50 sticky bottom-0 z-10">
            <button className="flex-1 py-4 bg-slate-200 text-slate-600 font-bold rounded-2xl hover:bg-slate-300 transition active:scale-95">Print Label</button>
