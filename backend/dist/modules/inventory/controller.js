...
export class InventoryController {
    ...
    static async createCategory(req, res, next) {
        try {
            ...
            if (error.code === 11000)
                next(new ApiError(400, "Category already exists"));
            else
                next(error);
        }
    }
    ...
    // --- Items ---
    static async getItems(_req, res, next) {
        try {
    ...
    static async createItem(req, res, next) {
        try {
            ...
            if (!val.isValid)
                throw new ApiError(400, "Validation failed", val.errors);
            ...
            if (error.code === 11000)
                next(new ApiError(400, "Item ID already exists"));
            else
                next(error);
        }
    }
